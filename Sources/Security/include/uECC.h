...
*/
typedef int (*uECC_RNG_Function)(uint8_t *dest, uint16_t size);
...
typedef struct uECC_HashContext {
    int (*init_hash)(struct sha_ctx* ctx, uint16_t method);
    void (*update_hash)(struct sha_ctx* ctx, uint8_t* data, uint16_t len);
    void (*finish_hash)(struct sha_ctx* ctx, uint8_t* digest);
    unsigned block_size; /* Hash function block size in bytes, eg 64 for SHA-256. */
    unsigned result_size; /* Hash function result size in bytes, eg 32 for SHA-256. */
    uint8_t tmp[128]; /* Must point to a buffer of at least (2 * result_size + block_size) bytes. */
    struct sha_ctx ctx;
} uECC_HashContext;
